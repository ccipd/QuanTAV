...
while(true)
    ...
    [T,Y] = msfm(SpeedImage, SourcePoint, true, true);
      
    % Trace a branch back to the used sourcepoints
    StartPoint=maxDistancePoint(Y,I,IS3D);
    
    ShortestLine=shortestpath(T,StartPoint,SourcePoint,.5,'rk4');
...
BoundaryDistance = msfm(SpeedImage, SourcePoint, true, true);
