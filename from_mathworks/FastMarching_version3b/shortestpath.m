...
while(true)
    %% edited shortpath computation to prevent stalling out to NaNs
    % if end point cannot be computed, halve step size by half until
    % receive valid endpoint
    % NMB 11/20/2019
    EndPoint = NaN(size(StartPoint)); 
    Stepsize=2*Stepsize0; 
    while any(isnan(EndPoint))
